...
const Review = () => {
    ...
    const leftClickHandler = () => {
        animate(slides.current[index], { scale: 1, rotate: 0 },
            { delay: 0.2 }
            ...
            <div className="flex flex-col items-center justify-center">
                <motion.div initial={{ opacity: 0, x: -200 }}
                    ...
                    {reviewsData.map((review, i) => (
                        <motion.div key={i}
                            initial={{ x: '100%' }}
                            className="absolute inset-0 flex flex-col items-center justify-center gap-y-7 lg:gap-y-4 border border-yellow-500 bg-zinc-50 p-14 lg:p-5 rounded-xl dark:bg-zinc-700 transition-colors" ref={(el) => slides.current.push(el)}>
                            <Image src={review.image} alt="Review Image" width={130} height={130} className="w-[130px] aspect-square rounded-full border border-yellow-500 p-4 object-contain" />
                            ...
                            <div className="flex flex-col items-center justify-center gap-y-2">
                                ...
                                <div className="flex items-center gap-x-2 text-2xl text-yellow-500">
                                    {review.stars.map((start, i) => (
                                        <span key={i}>{start === 1 ? starIcons[0] : starIcons[1]}</span>
