        ...
        <div id="experience" className="relative py-20 ">
            ...
            <div ref={containerRef} className="relative w-full h-full flex flex-col items-center justify-center gap-y-10 lg:gap-y-20 py-10">
                {experienceData.map((data, i) => (
                    <div  key={`id-${i}`} className={` w-[600px] xl:w-[480px] sm:w-full px-12 sm:px-0 relative -left-[300px] ${i % 2===0 ? '-left-[300px] xl:-left-[240px] lg:-left-0' : 'left-[300px] xl:left-[240px] lg:left-0'} `}>

                        <motion.div
                         initial={{ opacity: 0, x: i % 2 === 0 ? -80 : 80 }}
                          whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.7, type: 'spring', stiffness: 50 }} 
                            ...
                            <h1 className="text-xl sm:text-lg font-light text-gray-700 dark:text-white transition-colors">{data.title}</h1>
                            ...
                            <div className="text-gray-800 dark:text-gray-200 transition-colors">
                                ...
                                <ul className="pl-2">
                                    {data.experience.map((exp, j) => (
                                        <li className="my-1 font-extralight">{exp}</li>
                                    ))}
                                </ul>
                            </div>
                        ...
                        <div className={`w-14 absolute top-20  border border-gray-300 rounded-full aspect-square grid place-items-center text-red-400 font-light bg-white z-10 lg:hidden ${i % 2 === 0 ? 'left-full -translate-x-1/2' : 'right-full translate-x-1/2 lg:right-1/2'}`}>{date - experienceData.length + i + 1}</div>
                    </div>
                ))}
